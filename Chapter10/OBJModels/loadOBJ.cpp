...
// one glMultiDrawElements(primitive, *countIndices, type, **indices, countPrimitives) call -
// see Sec. 3.1.
//
// OUTPUT: vertex x/y/z coordinate values are written, one vertex after another, into the 
...
// actual number of faces is written into countFaces. The coordinates of the center of the
// object's bounding box are written int0 boxCenterX, boxCenterY, boxCenterZ.
	...
	float boxCenter[3])
{
	// Temp variables.
	std::string line;
	int vertexCoordNo = 0, indexArrayNo = 0;
	...
	while (getline(inFile, line)) // Read successive lines.
	{
		...
		if (line.substr(0, 2) == "v ") // Vertex line.
		{
			...
			// The (optional) w value is not read frpm the file. 
			for (int i = 0; i < 3; i++)
			{
				currentString >> coordVal;
				vertices[vertexCoordNo++] = coordVal;
